...
const Dashboard = () => {
  ...
  const checkActiveDeposit = async () => {
    try {
      ...
      // LOGIC: Check if active AND if the status is 'pending'
      if (response.data.active && response.data.deposit) {
        const status = response.data.deposit.status?.toLowerCase().trim();
        
        if (status === 'pending') {
          setActiveDeposit(response.data.deposit);
        } else {
          // If status is 'processing', hide the banner
    ...
    <div className="min-h-screen bg-[#F3F4F6] flex overflow-x-hidden">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="p-4 md:p-8 max-w-7xl w-full mx-auto space-y-8">
          ...
          {/* --- PENDING BANNER (Only shows when status is 'pending') --- */}
          {activeDeposit && (
...
const InvestmentCard = ({ pkg, onInvest, isBuying }) => (
  <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
    ...
    <p className="text-gray-400 text-xs mb-6 font-bold uppercase">Daily yield: {Number(pkg.daily_yield).toFixed(1)}%</p>
